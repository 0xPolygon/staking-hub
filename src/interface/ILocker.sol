...
interface ILocker {
    /// @notice Indicates change in balance. Monitored by services offchain.
    /// @dev Must be logged on balance change (depositing, initiating withdrawal, slashing, etc.).
    /// @param staker Staker.
    /// @param newBalance New balance.
    event BalanceChanged(address staker, uint256 newBalance);

    /// @dev Should be logged on withdrawal.
    /// @param staker Staker.
    /// @param amount Amount.
    event Withdrawn(address staker, uint256 amount);

    /// @notice Does internal accounting. Should perform risk managment.
    ...
    /// @notice Must burn slashed funds. Must aggregates slashings by applying percentage to balance at start of freeze period. Must burn funds scheduled for withdrawal first.
    /// @dev Triggered by hub.
    /// @dev Logs `BalanceChanged`.
    /// @param staker Staker.
    /// @param service Service.
