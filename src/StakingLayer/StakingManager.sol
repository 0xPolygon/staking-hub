...
contract StakingManager is StakingLayerManager {
    ...
    function restake(uint256 serviceId, uint256[] calldata amountsOrIds, uint256 commitUntil) external {
        ...
        for (uint256 i; i < strategyIds.length; ++i) {
            ...
            try strategyAddresses[strategyIds[i]].onRestake(msg.sender, amountsOrIds[i], serviceId, commitUntil, maximumSlashingPercentages.get(i)) {}
        ...
        returns (bool finalized, uint256 finalizationTime)
    {
        ...
        for (uint256 i; i < strategyIds.length; ++i) {
            try strategyAddresses[strategyIds[i]].onUnstake(msg.sender, serviceId, amountsOrIds[i]) {}
            catch (bytes memory data) {
        ...
        for (uint256 i; i < serviceData[serviceId].strategies.length; ++i) {
            try strategyAddresses[serviceData[serviceId].strategies[i]].balanceOfIn(msg.sender, serviceId) returns (uint256 balanceOfInService) {
